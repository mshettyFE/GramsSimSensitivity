...
int push_to_map(EntryKey &key, E& values, std::map< EntryKey, std::vector<std::unique_ptr<E>> > &Series){
  // Push key,value pair to map
    if(Series.count(key)==0){
        ...
        Events.push_back(std::make_unique<E>(values) );
        // Problem with move semantics here...
        Series[key] = std::move(Events);
    }
    else{
        Series[key].push_back(std::make_unique<E>(values));
    }
  return 0;
}


std::map<EntryKey, std::vector<std::unique_ptr<GramsG4Entry>> > ReadGramsG4(std::string GramsG4FileName, bool verbose){
  // See type defs for G4Entry
  // Uses <run,event> as key, and a list of interactions as the value
    std::map<EntryKey, std::vector<std::unique_ptr<GramsG4Entry> > > mapping;
...
std::map<EntryKey, std::vector<std::unique_ptr<GramsDetSimEntry>> > ReadGramsDetSim(std::string GramsDetSimFileName,bool verbose){
  // Open Relevant DetSim data
  // Format of DetSimMap
  ROOT::RDataFrame DetSim( "DetSim", GramsDetSimFileName, {"Run", "Event", "TrackID","energy",
    "x","y","z","timeAtAnode"} );

  std::map<EntryKey, std::vector<std::unique_ptr<GramsDetSimEntry> > >  DetMapping;
...
std::map<EntryKey, std::vector<std::unique_ptr<GramsExtractEntry>> > ReadExtract(std::string ExtractFileName,bool verbose){
  ...
  std::map<EntryKey, std::vector<std::unique_ptr<GramsExtractEntry> > >  Output;

  Series.Foreach(
  // Shove data into map depending on series type
  [&Output,verbose](int run, int event, double energy, double t, double x, double y, double z,
  double tDet, double xDet, double yDet, double zDet, double DetEnergy, std::string SeriesType)
  {
    ...
    GramsExtractEntry observables = GramsExtractEntry(true, run, event, energy, t,x,y,z, DetEnergy, tDet, xDet, yDet, zDet, EventType);
...
std::map<EntryKey, std::unique_ptr<GramsRecoEntry>> ReadReconstructFromSkyMap(std::string ReconstructName, bool verbose){
  ...
  std::map<EntryKey, std::unique_ptr<GramsRecoEntry> >  Output;
  ...
  {
    GramsRecoEntry observables  = GramsRecoEntry(run, event, xDir, yDir, zDir, xTip, yTip, zTip, RecoAngle, TruthEnergy);
    EntryKey key = observables.extract_key();
    Output[key] = std::make_unique<GramsRecoEntry>(observables);
    if(verbose){
      observables.print();
    }
 },

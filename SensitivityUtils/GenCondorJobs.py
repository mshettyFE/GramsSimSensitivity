...
def cmd_script_generation_skymap(output_directory_base_path, Job, cmd_name, shell_name,tar_name, nbatches):
    home = os.getcwd()
    shell_path = os.path.join(home,shell_name)
    tar_path = os.path.join(home,tar_name)
    condor_output = "temp-$(Process).out"
...
## If the directory already exists, then keep going
    ...
    if not (os.path.exists(data_dir)):
        ...
        sys.exit()
    if len(glob.glob(os.path.join(data_dir,"*_Reco_*.root"))) == 0:
